...
const { commitment, proof } = zkp.generateProof(SECRET_ID, nonce);
...
console.log(`   - Random Challenge Response (z_v): ${proof.z_v.substring(0, 16)}...`);
...
const tamperedProof = { ...proof, z_v: BigInt(proof.z_v) + 1n }; // Change the response slightly
const isTamperedValid = zkp.verifyProof(commitment, tamperedProof, nonce);
