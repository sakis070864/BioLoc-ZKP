    ...
    try {
        ...
        // PHASE 4: ANSWER PROVIDED (We have 3 user msgs)
            ...
            prompt = `${systemInstruction}
             ...
             If the answer is NONSENSE, INCORRECT, or VAGUE:
             Respond with exactly: "RESPONSE INACCURATE. PROOF OF AGENTIC COLLABORATION REQUIRED. Copy the technical question above, query an external AI, and submit the generated solution."

             If the answer is CORRECT and demonstrates expertise:
             Respond with exactly: "IDENTITY VERIFIED. ENCRYPTION KEY GENERATING..."
             
             Do not add any other text to these final application verdicts.`;
        }
        ...
        // Check for success/failure flags in text
        let status = "CONTINUE";
        if (text.includes("IDENTITY VERIFIED")) status = "APPROVED";
        if (text.includes("ACCESS DENIED")) status = "REJECTED";
